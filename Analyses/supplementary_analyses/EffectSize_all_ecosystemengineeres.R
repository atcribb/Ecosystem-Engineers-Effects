...
#Summary: Calculates effect size of reef-building AND bioturbating ecosystem engineers on generic richness, Shannon's Diversity, and Simpson's Dominance for each stage through the Phanerozoic
...
reef_formations <- unique(all_reef_builders$formation) #formations containing reef builders 
reef_genera <- unique(all_reef_builders$genus) #reef-builder genera

load('Data/Bioturbators_data.RData') #bioturbator data
bioturbator_genera <- unique(bioturbators_data$genus) #get each ecosystem engineering genus name
bioturbator_formations <- unique(bioturbators_data$formation) #get each formation name containing ecosystem engineers

ecoeng_genera <- c(reef_genera, bioturbator_genera)
ecoeng_formations <- unique(c(reef_formations, bioturbator_formations))
...
save(all_results_df, file='Output/allecosystemengineers_effectsizes.RData')
