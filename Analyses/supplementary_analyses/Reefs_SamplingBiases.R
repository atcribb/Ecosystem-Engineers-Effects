...
effort_diversity_x2 <- ggplot(data=subset(compare_subsampling, !(is.na(HedgesG_H)))) +
  ...
  geom_point(aes(x=sampling_difference_colls, y=HedgesG_H, 
                 fill=method, shape=method), size=2, colour='black') +
  geom_text(data=subset(R2_x2_text, measure=="Shannon's Diversity"), x=-5, y=1.5, hjust=0, aes(label=paste0("R^2~'='~", r2)), parse = TRUE) +
  facet_wrap(vars(method)) +
  scale_fill_manual(values=compare.cols) +
  scale_shape_manual(values=compare.shapes) +
  scale_color_manual(values=compare.cols) +
...
effort_diversity_x1 <- ggplot(data=subset(compare_subsampling, !(is.na(HedgesG_H)))) +
  ...
  geom_point(aes(x=sampling_difference_colls, y=HedgesG_H, 
                 fill=method, shape=method), size=2, colour='black') +
  geom_text(data=subset(R2_x1_text, measure=="Shannon's Diversity"), x=-5, y=1.5, hjust=0, aes(label=paste0("R^2~'='~", r2)), parse = TRUE) +
  facet_wrap(vars(method)) +
  scale_fill_manual(values=compare.cols) +
  scale_shape_manual(values=compare.shapes) +
  scale_color_manual(values=compare.cols) +
...
FigureS7 <- ggarrange2(effort_diversity_x1, effort_diversity_x2, ncol=1)
