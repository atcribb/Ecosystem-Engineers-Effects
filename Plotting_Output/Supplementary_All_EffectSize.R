...
setwd("/Users/alisontcribb/Desktop/Manucripts/EcosystemEngineering_Biodiversity/Revision/Revision_1")
...
#==== load data ====#
load('Revision_analyses/Output/allecosystemengineers_effectsizes.RData')
...
compare_richness_plot <- ggplot(data=compare_richness) +
  ...
  scale_y_continuous(limits=c(0,30), name="Generic Richness") +
  coord_geo(pos='bottom', dat='periods', size=3, abbrv=TRUE, height=unit(1,'line')) +
  annotate("text", x=530, y=28.5, label="A", size=3) +
  theme_classic() +
  effectsize_theme
...
effectsize_H_plot <- ggplot(data=subset(results_df, !is.na(HedgesG_H))) +
  ...
  scale_fill_manual(values=period.cols) +
  scale_x_reverse(limits=c(538,-5), name='Time (mya)') +
  scale_y_continuous(limits=c(-1.5,2.8), name=expression(paste("Hedges' g (\u00b11",sigma,")"))) +
  guides(fill='none') +
  coord_geo(pos='bottom', dat='periods', size=3, abbrv=TRUE, height=unit(1,'line')) +
  annotate("text", x=530, y=2.6, label="B", size=3) +
  theme_classic() +
  effectsize_theme +
...
all_ecosystemengineer_effects <- ggarrange2(compare_H_plot, effectsize_H_plot, ncol=1)
ggsave(filename='Revision_analyses/Figures/all_ecosystemengineers_effectsize_H.pdf', plot=all_ecosystemengineer_effects,
       height=5.5, width=6.53, unit='in')
